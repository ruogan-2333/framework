...
# -*- coding: utf-8 -*-
"""
Synchronous tester for GPTClient.propose_navigation using saved snap.json files.

Input:
- snap.json files produced by test_debug/capture_and_process_debug.py under
  test_debug/intermediate/<run_id>/<image_name>/snap.json.

Output:
- For each snap, writes llm_navigation_input.json and
  llm_navigation_result.json under test_debug/llm_navigation_output.
...
def setup_logging(debug: bool) -> None:
    """
    Input: debug flag.
    Output: configures process-wide logging for this LLM test script.
    """
...
def draw_llm_actions_overlay(snap: Dict[str, Any], result: Dict[str, Any], out_path: Path) -> None:
    """
    ...
    for label, step, color in iter_action_steps_for_overlay(result):
...
def build_input_record(snap_path: Path, snap: Dict[str, Any]) -> Dict[str, Any]:
    """
    Input: snap path and loaded snap dictionary.
    Output: compact Navigation LLM input summary without screenshot base64.
    """
...
def build_ui_digest(ui_json: Dict[str, Any]) -> Any:
    ...
    Output: compact UI digest using the same limit as GPTClient.propose_navigation.
    """
...
def build_gpt_client() -> GPTClient:
    ...
    Output: initialized GPTClient for synchronous Navigation calls.
    """
...
def process_one_snap(gpt: GPTClient, snap_path: Path, output_run_dir: Path) -> bool:
    """
    Input: GPTClient, one snap.json path, and output run directory.
    Output: returns True on successful LLM result; writes input/result/failure JSON.
    """
    snap = read_json(snap_path)
    image_token = safe_token(snap_path.parent.name)
    out_dir = output_run_dir / image_token
    input_record = build_input_record(snap_path, snap)
...
def main() -> int:
    ...
    print("=== LLM Navigation synchronous test ===")
    ...
    results = [process_one_snap(gpt, snap_path, output_run_dir) for snap_path in snap_files]
